...
(function () {
    ...
    function processOneIndd(inddFile, logs) {
        ...
        var snapDir = Folder(doc.fullName.parent.fsName + "/_snapshots");
    ...
    function main() {
        ...
        if (specificInddPath) {
            ...
            if (!inddFile.exists) {
                ...
                processOneIndd(inddFile, logs);
            }
        } else if (projectRoot) {
            // 自动扫描 workspace 下所有 .indd
            var workspaceFolder = Folder(projectRoot.fsName + "/workspace/B_outputs");
            if (!workspaceFolder.exists) {
                pushLog(logs, "B_outputs 目录不存在: " + workspaceFolder.fsName);
            } else {
                var inddFiles = [];
                ...
                for (j = 0; j < inddFiles.length; j += 1) {
                    if (processOneIndd(inddFiles[j], logs)) {
                        successCount += 1;
                    } else {
                        failCount += 1;
                    }
